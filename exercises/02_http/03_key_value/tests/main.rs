...
fn make_assign_request(input: &str) {
    let out = Command::new("curl")
        .args([
            "--request",
            "POST",
...
#[test]
fn assign() {
    make_assign_request("a = 10");
    make_assign_request("b = 10 * (7 + a)");
    assert_eq!(make_request("b"), 170.0);
}

#[test]
fn reassign() {
    make_assign_request("a = 10");
    make_assign_request("a = 3 * (7 + a)");
    assert_eq!(make_request("a"), 51.0);
}
